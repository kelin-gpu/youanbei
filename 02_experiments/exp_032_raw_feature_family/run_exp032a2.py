...
"""exp032a2: corrected incrementality diagnostic (partial correlation measure).

Repeats the exp032a scan but accumulates, per section, the three correlations
needed for the partial-correlation numerator (r_fy, r_fb, r_by) and evaluates
the preregistered amended gates. Zero training, no prediction output.
"""
...
RESULT = ROOT / "04_results" / "exp_032a2_partial_correlation_diagnostic"
SLICE_START = 1885
...
def zscore_features(raw: np.ndarray, t_local: int, window: int) -> np.ndarray:
    start = max(0, t_local - window)
    history = raw[start:t_local]
    current = raw[t_local]
...
def main() -> int:
    ...
    print("[diag] building OOF proxies", flush=True)
    ...
    sums = {w: {"fy": np.zeros(n_features), "fb": np.zeros(n_features), "by": 0.0}
            for w in ("fold2", "fold3", "valid")}
    counts = {w: 0 for w in ("fold2", "fold3", "valid")}
    valid_partial_series: dict[int, list[float]] = {}
    ...
    for window_name, ((start, stop), split) in windows.items():
        ...
        for time_index in range(start, stop):
            ...
            if t_local not in z_cache:
                z_cache[t_local] = {w: zscore_features(raw, t_local, w) for w in Z_WINDOWS}
                for k in [k for k in z_cache if k < t_local - max(Z_WINDOWS)]:
                    del z_cache[k]
            if split == "train":
                group_index = time_to_group[time_index]
                rows_idx = positions[group_index]
                stocks = train_stocks[rows_idx]
            ...
            y_rank = rank01_nan(y)
            base_rank = rank01_nan(base)
            r_by = corr_finite(base_rank, y_rank)
            if np.isfinite(r_by):
                sums[window_name]["by"] += r_by

            columns: list[tuple[int, np.ndarray]] = []
            for j in UNUSED59:
                columns.append((feature_index[f"F1_rank_num{j}"],
                                rank01_nan(raw[t_local][:, j].astype(np.float32))[stocks]))
            for w in Z_WINDOWS:
                z = z_cache[t_local][w]
                tag = "F2_z20" if w == 20 else "F3_z60"
                for j in range(99):
                    columns.append((feature_index[f"{tag}_num{j}"], z[:, j][stocks]))
            partial_row = np.full(n_features, np.nan, dtype=np.float64)
            for idx, col in columns:
                r_fy = corr_finite(col, y_rank)
                r_fb = corr_finite(col, base_rank)
                if np.isfinite(r_fy):
                    sums[window_name]["fy"][idx] += r_fy
                if np.isfinite(r_fb):
                    sums[window_name]["fb"][idx] += r_fb
                if np.isfinite(r_fy) and np.isfinite(r_fb) and np.isfinite(r_by):
                    partial_row[idx] = r_fy - r_fb * r_by
            if window_name == "valid":
                for idx in range(n_features):
                    if np.isfinite(partial_row[idx]):
                        valid_partial_series.setdefault(idx, []).append(float(partial_row[idx]))
    ...
    for w in ("fold2", "fold3", "valid"):
        ...
        sums[w]["fb"] /= counts[w]
        sums[w]["by"] /= counts[w]
        sums[w]["partial"] = sums[w]["fy"] - sums[w]["fb"] * sums[w]["by"]

    rows = []
    for name in all_features:
        idx = feature_index[name]
        pooled_raw = 0.5 * (sums["fold2"]["fy"][idx] + sums["fold3"]["fy"][idx])
        sign_ok = (np.sign(sums["fold2"]["fy"][idx]) == np.sign(sums["fold3"]["fy"][idx])
                   == np.sign(pooled_raw) != 0)
        rows.append({
            "feature": name,
            "rawIC_fold2": float(sums["fold2"]["fy"][idx]),
            "rawIC_fold3": float(sums["fold3"]["fy"][idx]),
            "rawIC_pooled": float(pooled_raw),
            "rawIC_valid": float(sums["valid"]["fy"][idx]),
            "basecorr_fold2": float(sums["fold2"]["fb"][idx]),
            "basecorr_valid": float(sums["valid"]["fb"][idx]),
            "partial_fold2": float(sums["fold2"]["partial"][idx]),
            "partial_fold3": float(sums["fold3"]["partial"][idx]),
            "partial_valid": float(sums["valid"]["partial"][idx]),
            "pool_pass": bool(sign_ok and abs(pooled_raw) >= RAW_IC_FLOOR),
        })
    with (RESULT / "feature_partial_table.csv").open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(rows[0]))
        writer.writeheader()
        writer.writerows(rows)

    pool = [r for r in rows if r["pool_pass"]]
    qualified = []
    for r in pool:
        p2, p3, pv = r["partial_fold2"], r["partial_fold3"], r["partial_valid"]
        if not all(np.isfinite(v) for v in (p2, p3, pv)):
            continue
        if np.sign(p2) == np.sign(p3) == np.sign(pv) != 0 and min(abs(p2), abs(p3), abs(pv)) >= PARTIAL_FLOOR:
            qualified.append(r)

    if qualified:
        aligned_valid_mean = float(np.mean([np.sign(r["partial_valid"]) * r["partial_valid"]
                                            for r in qualified]))
        series_sum: dict[int, float] = {}
        for r in qualified:
            direction = np.sign(r["partial_valid"])
            for k, v in enumerate(valid_partial_series.get(feature_index[r["feature"]], [])):
                series_sum[k] = series_sum.get(k, 0.0) + direction * v
        series = np.asarray([series_sum[k] / len(qualified) for k in sorted(series_sum)])
        ...
        aligned_valid_mean = worst_block = loo_ratio = float("nan")

    hashes_after = {name: sha256(path) for name, path in PROTECTED.items()}
    gates = {
        "g1_signal_pool_exists": bool(len(pool) >= POOL_MIN),
        "g2_partial_signal": bool(len(qualified) >= QUALIFIED_MIN),
        "g3_valid_transfer": bool(np.isfinite(aligned_valid_mean) and aligned_valid_mean > 0),
    ...
    metrics = {
        ...
        "family_breakdown": {
            ...
            "qualified_F1": len([r for r in qualified if r["feature"].startswith("F1")]),
            "qualified_F2": len([r for r in qualified if r["feature"].startswith("F2")]),
            "qualified_F3": len([r for r in qualified if r["feature"].startswith("F3")]),
        },
        "base_ic_by_window": {w: float(sums[w]["by"]) for w in ("fold2", "fold3", "valid")},
        "qualified_features": [
            {"feature": r["feature"], "rawIC_pooled": round(r["rawIC_pooled"], 5),
             "basecorr_valid": round(r["basecorr_valid"], 5),
             "partial_valid": round(r["partial_valid"], 5)} for r in qualified],
        "aligned_valid_partial_mean": aligned_valid_mean,
    ...
    print(json.dumps({k: metrics[k] for k in
                      ("decision", "gates", "counts", "family_breakdown", "base_ic_by_window",
                       "aligned_valid_partial_mean", "valid_worst32_block",
